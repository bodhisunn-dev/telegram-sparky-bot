...
serve(async (req) => {
  ...
  try {
    ...
    // Handle video generation from text
    if (configMap.image_gen_enabled && messageText.startsWith('/video')) {
      const prompt = messageText.replace('/video', '').trim();
      if (prompt) {
        const statusMessage = `Generating video: "${prompt}"... (This may take 1-2 minutes)`;
        ...
        const { data: videoData, error: videoError } = await supabase.functions.invoke('generate-video', {
          body: { prompt, type: 'text-to-video' }
        });

        if (videoError) {
          console.error('Video generation error:', videoError);
          await sendTelegramMessage(chatId, 'Sorry, I had trouble generating that video. Please try again!');
        } else if (videoData?.video) {
          // Send video back to Telegram
          await sendTelegramVideo(chatId, videoData.video);
          
          // Store success message
          await supabase.from('messages').insert({
            telegram_user_id: userData?.id,
            chat_id: chatId,
            message_text: '✅ Video generated successfully',
            is_bot_message: true
          });
        }
      } else {
        await sendTelegramMessage(chatId, '📹 Please provide a prompt!\n\nExample: /video a cat dancing in the rain');
      }
      return new Response(JSON.stringify({ ok: true }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    ...
    if (messageText.startsWith('/rvideo')) {
      ...
      if (message.photo && message.photo.length > 0) {
        ...
        if (fileData.ok) {
          ...
          const statusMessage = 'Generating video from image... (This may take 1-2 minutes)';
          await sendTelegramMessage(chatId, `🎬 ${statusMessage}`);
          ...
          // Call video generation function
          const { data: videoData, error: videoError } = await supabase.functions.invoke('generate-video', {
            body: { imageUrl: fileUrl, type: 'image-to-video' }
          });

          if (videoError) {
            console.error('Video generation error:', videoError);
            await sendTelegramMessage(chatId, 'Sorry, I had trouble generating video from that image. Please try again!');
          } else if (videoData?.video) {
            // Send video back to Telegram
            await sendTelegramVideo(chatId, videoData.video);
        ...
        await sendTelegramMessage(chatId, '📸 Please upload an image with the /rvideo command!\n\nExample: Send a photo with caption "/rvideo"');
      }
...
async function sendTelegramVideo(chatId: number, videoUrl: string) {
  const botToken = Deno.env.get('TELEGRAM_BOT_TOKEN');
  if (!botToken) {
    console.error('TELEGRAM_BOT_TOKEN not set');
    return;
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Telegram sendVideo error:', response.status, errorText);
    }
  } catch (error) {
